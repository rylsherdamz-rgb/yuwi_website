...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
      <section className="relative isolate">
        ...
        <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 pb-14 pt-20 sm:px-8 sm:pb-16 sm:pt-24 lg:px-12">
          <header className="animate-fade-up flex flex-col gap-4 rounded-[1.75rem] border border-[var(--border)] bg-[rgba(255,255,255,0.68)] px-4 py-4 shadow-[0_12px_40px_rgba(43,30,24,0.06)] backdrop-blur sm:flex-row sm:items-center sm:justify-between sm:rounded-full sm:px-6 dark:bg-[rgba(33,24,18,0.7)]">
            ...
            <a
              href="#download"
              className="inline-flex min-h-11 w-full items-center justify-center rounded-full bg-[var(--foreground)] px-4 py-3 text-sm font-semibold text-[var(--background)] transition-transform duration-200 hover:-translate-y-0.5 sm:w-auto"
            >
              Get the app
            </a>
          </header>

          <div className="grid flex-1 items-center gap-10 py-12 sm:gap-12 sm:py-14 lg:grid-cols-[1.15fr_0.85fr] lg:gap-14 lg:py-20">
            <div className="max-w-2xl">
              <Reveal>
              <div className="inline-flex max-w-full items-center rounded-full border border-[var(--accent-border)] bg-[var(--accent-muted)] px-4 py-2 text-sm font-semibold text-[var(--accent-strong)]">
                Built for notes, PDFs, and quiz review
              </div>
              </Reveal>

              <Reveal delay={80}>
                <h1 className="mt-6 font-[family-name:var(--font-brand)] text-[2.9rem] font-black leading-[0.95] tracking-[-0.05em] sm:text-6xl lg:text-7xl">
                  Turn study clutter into a cleaner review flow.
                </h1>
              </Reveal>

              <Reveal delay={140}>
                <p className="mt-5 max-w-xl text-base leading-7 text-[var(--muted)] sm:mt-6 sm:text-xl sm:leading-8">
              ...
              <Reveal delay={200}>
                ...
                <a
                  href="#download"
                  className="inline-flex min-h-13 items-center justify-center rounded-full bg-[var(--accent)] px-6 py-4 text-base font-bold text-white shadow-[0_18px_40px_rgba(240,138,52,0.28)] transition-transform duration-200 hover:-translate-y-0.5"
                >
                  Download Yuwi
                </a>
                <a
                  href="#features"
                  className="inline-flex min-h-13 items-center justify-center rounded-full border border-[var(--border-strong)] bg-[var(--card)] px-6 py-4 text-base font-semibold text-[var(--foreground)] transition-colors duration-200 hover:bg-[var(--surface)]"
                >
                  See how it works
                </a>
                </div>
              ...
              <div className="animate-float relative w-full max-w-[430px] rounded-[1.75rem] border border-[var(--border)] bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(255,245,235,0.88))] p-3 shadow-[0_30px_90px_rgba(43,30,24,0.12)] sm:rounded-[2rem] sm:p-4 dark:bg-[linear-gradient(180deg,rgba(33,24,18,0.98),rgba(42,30,23,0.95))]">
                <div className="rounded-[1.4rem] border border-[var(--border)] bg-[var(--card)] p-4 sm:rounded-[1.6rem] sm:p-5">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="shrink-0 rounded-full bg-[var(--accent-muted)] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--accent-strong)] sm:text-xs">
                      Live flow
                    </div>
                  </div>

                  <div className="mt-5 rounded-[1.2rem] bg-[var(--surface)] p-4 sm:mt-6 sm:rounded-[1.4rem] sm:p-5">
                  ...
                  <div className="mt-4 grid gap-3">
                    {steps.map((step, index) => (
                      <div key={step} className="flex items-center gap-3 rounded-[1.2rem] border border-[var(--border)] bg-[var(--card)] px-4 py-4 sm:gap-4">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[var(--accent-muted)] text-sm font-black text-[var(--accent-strong)]">
                          {index + 1}
                        </div>
      ...
      <section id="features" className="mx-auto w-full max-w-7xl px-4 pb-6 sm:px-8 sm:pb-8 lg:px-12">
        <Reveal>
        <div className="rounded-[1.75rem] border border-[var(--border)] bg-[var(--card)] px-5 py-8 shadow-[0_22px_60px_rgba(43,30,24,0.06)] sm:rounded-[2rem] sm:px-8 sm:py-10 lg:px-10">
          <div className="max-w-2xl">
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-6 sm:px-8 sm:pb-8 lg:px-12">
        <Reveal>
          <div className="rounded-[1.75rem] border border-[var(--accent-border)] bg-[var(--accent-muted)] px-5 py-6 sm:rounded-[2rem] sm:px-8 sm:py-8">
            <p className="text-sm font-black uppercase tracking-[0.24em] text-[var(--accent-strong)]">
              Gemini note
            </p>
      ...
      <section id="roadmap" className="mx-auto w-full max-w-7xl px-4 pb-6 sm:px-8 sm:pb-8 lg:px-12">
        <Reveal>
          <div className="rounded-[1.75rem] border border-[var(--border)] bg-[var(--card)] px-5 py-8 shadow-[0_22px_60px_rgba(43,30,24,0.06)] sm:rounded-[2rem] sm:px-8 sm:py-10 lg:px-10">
            <div className="max-w-2xl">
              <p className="text-sm font-black uppercase tracking-[0.24em] text-[var(--accent-strong)]">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex min-h-13 items-center justify-center rounded-full bg-[var(--foreground)] px-6 py-4 text-base font-bold text-[var(--background)] transition-transform duration-200 hover:-translate-y-0.5"
                >
                  Download APK
                </a>
                <a
